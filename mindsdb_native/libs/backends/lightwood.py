...
#         # Sort each group by order_by column(s)
...
#                     while len(group_by_ts_map[group].iloc[i][order_col]) <= nr_samples:
#                         group_by_ts_map[group].iloc[i][order_col].append(0)
#             for order_col in order_by:
#                 group_by_ts_map[group][order_col] = group_by_ts_map[group][order_col].astype(object)

#                 for i, _ in enumerate(group_by_ts_map[group]):
#                     group_by_ts_map[group][order_col].iat[i] = [group_by_ts_map[group][order_col].iloc[i]]

#                     previous_indexes = [*range(max(i -  nr_samples, 0), i)]
